############################################################
# 8-POINT FFT — Radix-2 DIT Cooley-Tukey (No %hi/%lo)
# Computes FFT in-place with 3 stages
# Initializes all data at runtime
############################################################
...
############################################################
    ...
    lui  x6, 0x3f800            #  1.000000  → 0x3f800000
    sw   x6, 0(x1)

    li  x6, 0x3f35              #  0.70710677 → 0x3f3504f3
    slli x6, x6, 16
    li x31, 0x04f3
    add x6, x6, x31         
    sw   x6, 4(x1)

    lui  x6, 0x00000            #  0.0 → 0x00000000
    sw   x6, 8(x1)

    li  x6, 0xbf35            # -0.70710677 → 0xbe3504f3
    slli x6, x6, 16
    li x31, 0x04f3
    add x6, x6, x31 
    sw   x6, 12(x1)

    lui  x6, 0xbf800            # -1.0 → 0xbf800000
    sw   x6, 16(x1)
